...
> **Scope**: Single decision cycle in `DartAgent.decide()`.
...
Each decision produces a multi-turn conversation with the LLM:

```
[System]   System prompt (identity, rules, workflow, output schema)
[User]     Market state + portfolio + session + memory + chart images
...
Maximum rounds: `max_tool_calls + 2` (1 for analysis plan, N for tools, 1 for final signal).

---

## 2. System Prompt (Message 1)
...
### 2.2 Action Vocabulary (State-Aware)
```
...
### 2.4 Price-Action Workflow (8 Steps)
```
...
### 2.7 Non-Negotiable Constraints (10 rules)
```
1. No future data.
...
## 3. User Prompt — Initial Market State (Message 2)

**Source**: `core/context.py:format_market_state_for_prompt()` + `build_user_prompt()`

### 3.1 Market State Header
```
============================================================
MARKET STATE at 2026-05-28 10:00:00+05:30
...
### 3.4 Trend Summaries (from all candles, summarized)
```
TRENDS:
...
### 3.5 Price-Action Pattern and Location
```
PRICE-ACTION PATTERN: consolidation_after_move
PRICE LOCATION: near resistance
```

### 3.6 Indicators (computed from all intraday candles)
```
INDICATORS:
  RSI(14): 58.2 (neutral)
  ATR(14): 12.50
...
### 3.9 Raw Intraday Candles (last 8 of 75)
```
LAST 8 15min CANDLES (most recent first):
...
### 3.10 Raw Daily Candles (last 5 of 22)
```
LAST 5 DAILY CANDLES:
  Date           Open     High      Low    Close    Range
  2026-05-22  2380.00  2410.00  2375.00  2405.00    35.00
  2026-05-23  2405.00  2420.00  2398.00  2415.00    22.00
  2026-05-24  2415.00  2425.00  2408.00  2418.00    17.00
  2026-05-27  2418.00  2430.00  2412.00  2425.00    18.00
  2026-05-28  2425.00  2428.00  2420.00  2422.00     8.00
```

### 3.11 Raw Weekly Candles (last 3 of 13)
```
LAST 3 WEEKLY CANDLES:
  Week           Open     High      Low    Close    Range
  2026-05-05  2350.00  2385.00  2340.00  2380.00    45.00
  2026-05-12  2380.00  2410.00  2370.00  2405.00    40.00
  2026-05-19  2405.00  2430.00  2395.00  2420.00    35.00
```

### 3.12 Visual Context Pack (chart descriptions + paths)
```
VISUAL CONTEXT PACK:
...
## 4. Portfolio State (appended to user prompt)
...
## 5. Session State (appended to user prompt)
...
## 6. Memory Summary (appended to user prompt)
...
**After plan validation**: Replaced with focused retrieval (see Section 8).

---

## 7. Chart Images (base64-encoded)
...
### 7.1 Initial Message Charts (2 images)
...
### 7.2 Tool-Round Charts (0-N images)
...
## 8. Focused Memory Injection (after plan validation)
...
## 9. Tool Descriptions (appended to user prompt)
...
## 10. Tool Results (subsequent messages)
...
## 11. Final Reminder (when max tools reached)
...
```
                    Postgres
                    ┌─────────────────────────────┐
                    │ portfolio_snapshots          │──→ Portfolio summary
                    │ positions                    │──→ Open position details
                    │ session_maps                 │──→ Session state
                    ...
                    ┌─────────────────────────────┐
                    │ 75 intraday candles          │──→ 8 raw + indicators + swings + levels
                    │ 22 daily candles             │──→ 5 raw + trend summary
                    │ 13 weekly candles            │──→ 3 raw + trend summary
                    └─────────────────────────────┘

                    Charts (generated at T)
                    ┌─────────────────────────────┐
                    │ context_dashboard.png        │──→ base64 image
                    │ decision_zoom_chart.png      │──→ base64 image
                    ...
                    │  - Session (150 tok)        │
                    │  - Memory (300 tok)         │
                    │  - 16 raw candles (500 tok) │
                    │  - 2 chart images           │
                    │  - Tool descriptions (1K)   │
                    │  - Tool results (varies)    │
                    │                             │
                    │  Total: ~8-12K tokens/step  │
                    └─────────────────────────────┘
```

---

## 13. What the LLM Does NOT See

| Data | Why Not |
|---|---|
| Future candles (after T) | Lookahead prevention |
| Full 75 intraday candles as raw data | Too many tokens; summaries + tools cover it |
| Full 22 daily candles as raw data | Summaries cover it |
| Full 13 weekly candles as raw data | Summaries cover it |
...
## 14. Token Budget Estimate

| Component | Estimated Tokens |
|---|---|
| System prompt | ~4,000 |
| Market state text | ~2,000 |
| Portfolio summary | ~200 |
| Session summary | ~150 |
| Memory summary | ~300 |
| 16 raw candles (text tables) | ~500 |
| Tool descriptions | ~1,000 |
| Chart images (2x base64) | ~2,000 |
| **Initial message total** | **~10,000** |
| Per tool round (request + result) | ~500-1,500 |
| Focused memory injection | ~300 |
| Final reminder | ~100 |
| **Total per decision (3 tool rounds)** | **~12,000-15,000** |
